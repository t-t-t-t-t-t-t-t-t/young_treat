<template>
	<view class="body">
		...
		<!-- 搜索内容 -->
		<view class="searchPassage" v-show="ifSearched">
			<!-- 文章 -->
			<recmdPassage v-for="(passage,index) in passages" :key="passage.id"
			:passage_info="passage">
			</recmdPassage>
			<!-- 搜索结果为空时 -->
			<u-empty text="无搜索结果" mode="search" icon-size="180" font-size="32" margin-top="80" v-if="passages.length===0 && !flag"></u-empty>
		</view>
		
		<!-- 底部加载栏 -->
		<bottomLoading :isLoading="isloading" v-if="passages.length!==0"></bottomLoading>
...
<script>
	...
	export default {
		...
		onReachBottom() {
			...
			this.isloading = 1;
			//获取更多文章
			this.getPassage(this.pageNum,this.pageSize,this.searchText)
		},
		methods:{
			//清空搜索记录
			clear(){
			...
			goSearching(searchText){
				...
				this.flag = true;
				//获取文章数据
				this.getPassage(this.pageNum,this.pageSize,searchText);
				//将 搜索记录 进行本地存储
			...
			//请求搜索的 文章 数据
			getPassage(page,pageSize,search){
				uni.request({
